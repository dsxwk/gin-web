		...
		<template v-for="val in menuLists">
			<el-sub-menu :index="val.path" v-if="val.children && val.children.length > 0" :key="val.path">
				<template #title>
					...
					<span>{{ $t(val.meta.transKey || val.meta.title) }}</span>
				</template>
			...
			<template v-else>
				<el-menu-item :index="val.path" :key="val.path">
					...
					<template #title v-else>
						<a class="w100" @click.prevent="onALinkClick(val)">{{ $t(val.meta.transKey || val.meta.title) }}</a>
					</template>
				</el-menu-item>
			</template>
		</template>
...
const setParentHighlight = (currentRoute) => {
  ...
  // 如果路径分割后长度大于等于 4 且 meta.isHide 为 true，则返回父级路径
  if (pathSplit && pathSplit.length >= 4 && meta?.isHide) {
    return pathSplit.slice(0, 3).join('/');
  }
